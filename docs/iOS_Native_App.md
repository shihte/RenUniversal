...
### v1.3.1 (Current)
*   **[新功能] 統計數據工具 (Statistics Tool)**：新增全新網頁版與 iOS 內建工具，可即時追蹤四種錯誤姿勢（歪頭 tilt、轉頭 turn、駝背 slouch、前傾 lean）。
*   **[新功能] 可調式有效閾值**：支援即時過濾雜訊，預設 1.5 秒以上才判定為有效不良姿勢，可動態調整。
*   **[新功能] 圖表與清單呈現**：自動計算各類事件比例，並提供歷史判定清單；支援單次會話一鍵匯出 CSV。
*   **[隱私優化] 零持久化設計**：統計資料採無持久化（Non-persistent）設計，保護隱私不留存歷史紀錄。
